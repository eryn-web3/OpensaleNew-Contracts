...
import "./Interface8/IOdcPool.sol";

contract OdcFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 odcAtLastUserAction; // keeps track of odc deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Odc token
    IOdcPool public immutable odcPool; //Odc pool
    ...
    //When call odcpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositOdc(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _odcPool: OdcPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_odcPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Odc Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    odcPool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositOdc(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Odc Flexible Pool
    ...
    /**
     * @notice Withdraws from Odc Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking odc");
        staking = false;
        odcPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Odc Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in OdcPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = odcPool.userInfo(address(this));
        uint256 pricePerFullShare = odcPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into OdcPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            odcPool.deposit(bal, 0);
        }
    }
}