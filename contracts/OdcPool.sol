...
import "./Interface8/IMasterChefV2.sol";
import "./Interface8/IBoostContract.sol";
import "./Interface8/IVOdc.sol";
...
contract OdcPool is Ownable, Pausable {
    ...
    event NewVOdcContract(address VOdc);
     ...
     * @param _pid: odc pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(odcPoolPID, balance);
        emit Init();
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onOdcPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function depositOperation(
        ...
        if (VOdc != address(0)) {
            IVOdc(VOdc).deposit(_user, _amount, _lockDuration);
        }
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VOdc != address(0)) {
            IVOdc(VOdc).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending ODC tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingOdc = masterchefV2.pendingOdc(odcPoolPID, address(this));
        if (pendingOdc > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(odcPoolPID, 0);
    ...
    /**
     * @notice Set VOdc Contract address
     * @dev Callable by the contract admin.
     */
    function setVOdcContract(address _VOdc) external onlyAdmin {
        require(_VOdc != address(0), "Cannot be zero address");
        VOdc = _VOdc;
        emit NewVOdcContract(VOdc);
    }
