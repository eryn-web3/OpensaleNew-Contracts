...
/** @title OpensaleFinance Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract OpensaleFinanceLottery is ReentrancyGuard, IOpensaleFinanceLottery, Ownable {
    ...
    uint256 public maxPriceTicketInOdc = 50 ether;
    uint256 public minPriceTicketInOdc = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInOdc;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] odcPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInOdc;
        uint32 finalNumber;
    }
     ...
     * @param _odcTokenAddress: address of the ODC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _odcTokenAddress, address _randomGeneratorAddress) {
        odcToken = IERC20(_odcTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ODC to this contract
        uint256 amountOdcToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInOdc,
            _ticketNumbers.length
        );

        // Transfer odc tokens to this contract
        odcToken.safeTransferFrom(address(msg.sender), address(this), amountOdcToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInOdc += amountOdcToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInOdcToTransfer
        uint256 rewardInOdcToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInOdcToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        odcToken.safeTransfer(msg.sender, rewardInOdcToTransfer);

        emit TicketsClaim(msg.sender, rewardInOdcToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInOdc) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ODC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].odcPerBracket[j] =
                ...
                // A. No ODC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].odcPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInOdc - amountToShareToWinners);

        // Transfer ODC to treasury address
        odcToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        odcToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInOdc += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInOdc >= minPriceTicketInOdc) && (_priceTicketInOdc <= maxPriceTicketInOdc),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInOdc: _priceTicketInOdc,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            odcPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInOdc: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(odcToken), "Cannot be ODC token");
    ...
    /**
     * @notice Set ODC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInOdc: minimum price of a ticket in ODC
     * @param _maxPriceTicketInOdc: maximum price of a ticket in ODC
     */
    function setMinAndMaxTicketPriceInOdc(uint256 _minPriceTicketInOdc, uint256 _maxPriceTicketInOdc)
        external
        onlyOwner
    {
        require(_minPriceTicketInOdc <= _maxPriceTicketInOdc, "minPrice must be < maxPrice");

        minPriceTicketInOdc = _minPriceTicketInOdc;
        maxPriceTicketInOdc = _maxPriceTicketInOdc;
    }
     ...
     * @param _priceTicket price of a ticket (in ODC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].odcPerBracket[_bracket];
        } else {
            return 0;
        }
    }
