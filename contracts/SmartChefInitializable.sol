...
import "./Interface8/IOpensaleProfile.sol";
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Opensale profile
    IOpensaleProfile public immutable opensaleProfile;

    // Opensale Profile is requested
    bool public opensaleProfileIsRequested;

    // Opensale Profile points threshold
    uint256 public opensaleProfileThresholdPoints;
    ...
    /**
     * @notice Constructor
     * @param _opensaleProfile: Opensale Profile address
     * @param _opensaleProfileIsRequested: Opensale Profile is requested
     * @param _opensaleProfileThresholdPoints: Opensale Profile need threshold points
     */
    constructor(
        address _opensaleProfile,
        bool _opensaleProfileIsRequested,
        uint256 _opensaleProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        IOpensaleProfile(_opensaleProfile).getTeamProfile(1);
        opensaleProfile = IOpensaleProfile(_opensaleProfile);

        // if opensaleProfile is requested
        opensaleProfileIsRequested = _opensaleProfileIsRequested;

        // opensaleProfile threshold points when profile & points are requested
        opensaleProfileThresholdPoints = _opensaleProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!opensaleProfileIsRequested && opensaleProfileThresholdPoints == 0) ||
            opensaleProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (opensaleProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = opensaleProfile.getUserProfile(msg.sender);
        }

        require(
            opensaleProfileThresholdPoints == 0 || numberUserPoints >= opensaleProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        opensaleProfileIsRequested = _isRequested;
        opensaleProfileThresholdPoints = _thresholdPoints;
